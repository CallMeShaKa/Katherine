...
from Katherine import pbot as app
from Katherine import MONGO_DB_URI
from Katherine import BOT_ID
from Katherine import *
from Katherine.decorators.permissions import adminsOnly
from Katherine.utils.filter_groups import *
...
@app.on_message(
    filters.regex("Rosebot | @kingxlbot | sz rose | rose")
    & ~filters.bot
    & ~filters.via_bot
    & ~filters.forwarded
    & ~filters.reply
...
@app.on_message(filters.command("king"))
async def Lycia(_, message):
    if len(message.command) < 2:
        await message.reply_text("king Bot AI Voice Chatbot")
        return
    ...
    try:
        ...
        await m.edit(str(e))
        return
    await m.edit("Made By @theedecode...")
